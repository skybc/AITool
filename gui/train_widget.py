...
from PySide6.QtWidgets import (
    ...
    QComboBox, QLineEdit, QTextEdit, QFileDialog, QGroupBox,
    QGridLayout, QSpinBox, QDoubleSpinBox, QMessageBox, QProgressBar
)
...
from core.train import DefectDetector
...
class TrainThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.trainer = DefectDetector(self.config_path, self.output_dir)
            ...
            tee_writer = OutputLogger.create_tee_writer(
                string_buffer=self.output_buffer,
...
class TrainWidget(QWidget):
    ...
    def _validate_dataset(self, dataset_path):
        ...
        if not images_dir.exists():
            ...
            if not train_dir.exists():
                ...
                train_count = len([f for f in train_files if f.suffix.lower() in ['.jpg', '.png', '.jpeg']])
                if train_count == 0:
            ...
            if not val_dir.exists():
                ...
                val_count = len([f for f in val_files if f.suffix.lower() in ['.jpg', '.png', '.jpeg']])
                if val_count == 0:
            ...
            if test_dir.exists():
                ...
                test_count = len([f for f in test_files if f.suffix.lower() in ['.jpg', '.png', '.jpeg']])
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                if 'training' in config:
                    ...
                    self.batch_spin.setValue(config['training'].get('batch_size', 32))
                    self.lr_spin.setValue(config['training'].get('learning_rate', 0.001))
                    self.wd_spin.setValue(config['training'].get('weight_decay', 0.0005))
                    self.warmup_spin.setValue(config['training'].get('warmup_epochs', 5))
    ...
    def _save_config(self):
        ...
        try:
            ...
            config['training']['batch_size'] = self.batch_spin.value()
            config['training']['learning_rate'] = self.lr_spin.value()
